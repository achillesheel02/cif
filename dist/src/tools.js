...
import { encrypt as encryptFn, serializeBlob, decryptFromBase64, } from "./crypto.js";
import { createVault, unlockVault, lockVault, exportContinuation, fetchVaultIfMissing, isUnlocked, getActiveUser, listVaultUsers, readSeed, } from "./vault.js";
import { summarizeMoments, getRecentMoments } from "./db.js";
const VAULT_DIR = join(homedir(), ".cif-vault");
export async function unlockIdentity(params) {
...
export function lockIdentity(params) {
...
export function createIdentity(params) {
    ...
    const seedPreview = params.initial_seed.slice(0, 100) +
...
export function exportContinuationTool(params) {
    const userId = params.user_id ?? "barak";
...
export function rotatePassphrase(params) {
    const userId = params.user_id ?? "barak";
 ...
 */
export function detectAndDecrypt(params) {
