...
const activeVaults = new Map<string, { keyHex: string; saltHex: string }>();

function userDir(userId: string): string {
  return join(VAULT_DIR, userId);
}

function identityPath(userId: string): string {
  return join(userDir(userId), "identity.enc");
}

function sessionsLogPath(userId: string): string {
  return join(userDir(userId), "sessions.log");
}

function ensureVaultDir(userId: string): void {
 ...
 */
export async function fetchVaultIfMissing(
  userId: string,
  seedUrl: string
...
export function unlockVault(
  userId: string,
  passphrase: string
): { success: true; seed: string } | { success: false; error: string } {
  const path = identityPath(userId);
 ...
 */
export function exportContinuation(
  userId: string,
  passphrase: string,
  momentsSummary: string
): { success: true; blob: string } | { success: false; error: string } {
...
export function listVaultUsers(): string[] {
  ...
  try {
    return readdirSync(VAULT_DIR).filter((name: string) => {
...
function appendSessionLog(userId: string, event: string, detail: string): void {
